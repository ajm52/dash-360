...
def transcode(src: str, dst: str, config: dict):
    ...
    for ind, tile in enumerate(config["tiles"]):
        filter = "[0:v]crop=%d:%d:%d:%d[out%d]" % (
            tile["width"], tile["height"], tile["left"], tile["top"], ind)
    ...
    for tile_ind in range(tiles_num):
        ...
        adaptation_set = 'id=%d,descriptor=<SupplementalProperty schemeIdUri="urn:mpeg:dash:srd:2014" value="%s"/>,streams=%s' % (
            tile_ind, config["srd_values"][tile_ind], ",".join(output_streams))
    ...
    command = ["ffmpeg", "-i", src, "-filter_complex", ffmpeg_filter,
               *params, *adaptation_sets_params, "-f", "dash", dst]
    print(" ".join(command))
...
def main():
    ...
    parser.add_argument("config", type=str,
                        help="Configuration file for transcoding")
